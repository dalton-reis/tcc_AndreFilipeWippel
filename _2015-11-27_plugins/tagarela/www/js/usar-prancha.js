$(document).ready(function usarPrancha() {
    ...
    function okQuerySimbPranchaUso(tx, results) {
	    ...
	    if (len > 0) {    
	        for (var i=0; i<len; i++) {
		    	$(".simbolos").append("<li style='display:inline-block;'>"
									 +"<img src='img/"+results.rows.item(i).img+"' title='"+results.rows.item(i).audio+"' alt='"+results.rows.item(i).id+"' class='img-simbolo' style='margin:20px' height='150' width='150'/>"
									 +"</li>");
		    }
		}
		else {
		...
		$(".img-simbolo").click(function() {
    		...
    		srcAudio = appPath + "audio/" + $(this).attr("title");
