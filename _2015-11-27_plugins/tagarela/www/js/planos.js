$(document).ready(function() {
    ...
    function transSimbPlanoFil(tx) {
    	for (var i=0; i<planosFilId.length; i++) {
    		if (planosFilIdAux[i] == 0) {
    			tx.executeSql("SELECT img,audio,planos.id AS plano FROM simbolos,planos WHERE simbolos.id = ? AND planos.id = ?", [simbsPlanoFil[i],planosFilId[i]], okQuerySimbPlanoFil, nokQuery);
    			planosFilIdAux[i] = 1;
    			break;
    		}
    	}
    ...
    function okQuerySimbPlanoFil(tx, results) {
        ...
        for (var i=0; i<len; i++) {
	    	$(".planos").append("<li style='display:inline-block'><a href='../compartilhar/pranchas.html'>"
							   +"<img src='../img/"+results.rows.item(i).img+"' title='"+results.rows.item(i).audio+"' alt='"+results.rows.item(i).plano+"' class='img-plano' style='margin:20px' height='150' width='150'/>"
							   +"</a></li>");
	    }
	    $(".img-plano").mouseover(function() {
			var alt = $(this).attr("alt");
