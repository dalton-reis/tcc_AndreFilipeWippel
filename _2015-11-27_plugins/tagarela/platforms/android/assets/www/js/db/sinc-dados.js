...
function okConsSimbolos(tx, results) {
	...
	alert("qtdSimbLocal: "+qtdSimbLocal);
	var dados = {
		"aux" : 1
	};  
	$.ajax({
...
function baixarSimbolos() {
   	alert("baixarSimbolos");
	alert("qtdBaixar: "+qtdBaixar);
	var dados = {
		"qtdBaixar" : qtdBaixar
	};  
	$.ajax({
...
function okWebSimbolos(tx, results) {
	...
	for (var i=0; i<len; i++) {
		...
		alert("nome: "+simbsNomeLocal[i]);
		alert("img: "+imgWeb);
		alert("audio: "+audioWeb);
		var dados = {
			"id" : results.rows.item(i).id,
...
function uploadSimbolos(tx) {
	...
	for	(var i = 0; i < simbsNomeLocal.length; i++) {
		...
		alert("audioLocal: "+audioLocal);
		alert("audioWeb: "+audioWeb);
