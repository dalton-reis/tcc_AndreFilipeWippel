$(document).ready(function() {
	...
	function transFotoCons(tx) {
    	tx.executeSql("SELECT img,audio,categoria FROM simbolos WHERE simbolos.id = (SELECT simbolo FROM usuarios WHERE usuarios.id = ?)", [localStorage.idBuilder], okQueryFotoCons, nokQuery);
    }
    ...
    function transSimbCons(tx) {
    	for (var i=0; i<planosId.length; i++) {
    		if (planosIdAux[i] == 0) {
    			tx.executeSql("SELECT categoria,img,audio,planos.id AS plano FROM simbolos,planos WHERE simbolos.id = ? AND planos.id = ?", [simbsPlano[i],planosId[i]], okQuerySimbCons, nokQuery);
    			planosIdAux[i] = 1;
    			break;
    		}
    	}
    ...
    function okQuerySimbCons(tx, results) {
        ...
        for (var i=0; i<len; i++) {
	    	...
	    	if (localStorage.dinamico == "iOS") {
	    		$(".planos").append("<li><a href='usar-plano.html'>"
							   +"<img src='img/"+results.rows.item(i).img+"' title='"+results.rows.item(i).audio+"' alt='"+results.rows.item(i).plano+"' class='img-plano' style='border:10px solid "+corBorda+"' height='175' width='175' />"
							   +"</a></li>");
			}
	    	else {
	    		$(".planos").append("<li><a href='usar-plano.html'>"
							   +"<img src='"+results.rows.item(i).img+"' title='"+results.rows.item(i).audio+"' alt='"+results.rows.item(i).plano+"' class='img-plano' style='border:10px solid "+corBorda+"' height='175' width='175' />"
							   +"</a></li>");
			}
	    }
	    $(".img-plano").click(function() {
	...
	$(".foto-img").click(function mostrarInfo() {
    	...
    	srcAudio = $(this).children("img").attr("title");
