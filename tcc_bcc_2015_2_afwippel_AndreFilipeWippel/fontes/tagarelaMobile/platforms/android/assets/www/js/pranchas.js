    ...
    function transSimbPranchasPlanoFil(tx) {
    	for (var i=0; i<pranchasFilId.length; i++) {
    		if (pranchasFilIdAux[i] == 0) {
    			tx.executeSql("SELECT categoria,img,audio,pranchas.id AS prancha FROM simbolos,pranchas WHERE simbolos.id = ? AND pranchas.id = ?", [simbsPranchaFil[i],pranchasFilId[i]], okQuerySimbPranchasPlanoFil, nokQuery);
	...
	function okQuerySimbPranchasPlanoFil(tx, results) {
        ...
        for (var i=0; i<len; i++) {
	    	...
	    	if (localStorage.dinamico == "iOS") {
				$(".pranchas").append("<li style='display:inline-block'><a href='#'>"
								 +"<img src='../img/"+results.rows.item(i).img+"' title='"+results.rows.item(i).audio+"' alt='"+results.rows.item(i).prancha+"' class='img-prancha' style='margin:20px; border:10px solid "+corBorda+"' height='150' width='150'/>"
								 +"</a></li>");
	    	}
	    	else {
	    		$(".pranchas").append("<li style='display:inline-block'><a href='#'>"
								 +"<img src='"+results.rows.item(i).img+"' title='"+results.rows.item(i).audio+"' alt='"+results.rows.item(i).prancha+"' class='img-prancha' style='margin:20px; border:10px solid "+corBorda+"' height='150' width='150'/>"
								 +"</a></li>");
	    	}
	    }
	    $(".img-prancha").click(function reutilizarPrancha() {
	    ...
	    if (len > 0) {    
	        for (var i=0; i<len; i++) {
		    	if (results.rows.item(i).categoria != 1) {
					...
					if (localStorage.dinamico == "iOS") {
						ximg = "../img/"+results.rows.item(i).img;
					}
					else {
						ximg = results.rows.item(i).img;
					}
					xcat = results.rows.item(i).categoria;
				}
				else {
					xid = 0;
