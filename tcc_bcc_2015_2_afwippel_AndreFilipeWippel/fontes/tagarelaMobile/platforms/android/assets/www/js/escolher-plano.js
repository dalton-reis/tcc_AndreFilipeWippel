    ...
    function transSimbPlanoEsc(tx) {
    	for (var i=0; i<planosEscId.length; i++) {
    		if (planosEscIdAux[i] == 0) {
    			tx.executeSql("SELECT categoria,img,audio,planos.id AS plano FROM simbolos,planos WHERE simbolos.id = ? AND planos.id = ?", [simbsPlanoEsc[i],planosEscId[i]], okQuerySimbPlanoEsc, nokQuery);
    			planosEscIdAux[i] = 1;
    			break;
    		}
    	}
