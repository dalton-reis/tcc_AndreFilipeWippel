$(document).ready(function() {
	...
	function transFotoHome(tx) {
    	tx.executeSql("SELECT img,audio,categoria FROM simbolos WHERE simbolos.id = (SELECT simbolo FROM usuarios WHERE usuarios.id = ?)", [localStorage.idUser], okQueryFotoHome, nokQuery);
    }
    ...
    function transSimbHome(tx) {
    	for (var i=0; i<buildersId.length; i++) {
    		if (buildersIdAux[i] == 0) {
    			tx.executeSql("SELECT categoria,img,audio,usuarios.id AS usuid FROM simbolos,usuarios WHERE simbolos.id = (SELECT simbolo FROM usuarios WHERE usuarios.id = ?) AND usuarios.id = ?", [buildersId[i],buildersId[i]], okQuerySimbHome, nokQuery);
    			buildersIdAux[i] = 1;
    			break;
    		}
    	}
    ...
    function okQuerySimbHome(tx, results) {
        ...
        for (var i=0; i<len; i++) {
	    	...
	    	if (localStorage.dinamico == "iOS") {
	    		$(".builders").append("<li><a href='builder.html'>"
								 +"<img src='img/"+results.rows.item(i).img+"' title='"+results.rows.item(i).audio+"' alt='"+results.rows.item(i).usuid+"' class='foto-builder' style='margin-bottom:13px; border:10px solid "+corBorda+"' height='175' width='175' />"
								 +"</a></li>");
	    	}
	    	else {
	    		$(".builders").append("<li><a href='builder.html'>"
								 +"<img src='"+results.rows.item(i).img+"' title='"+results.rows.item(i).audio+"' alt='"+results.rows.item(i).usuid+"' class='foto-builder' style='margin-bottom:13px; border:10px solid "+corBorda+"' height='175' width='175' />"
								 +"</a></li>");
	    	}
	    }
	    $(".foto-builder").click(function() {
   	...
   	$(".foto-img").click(function mostrarInfo() {
    	var srcAudio = $(this).children("img").attr("title");
