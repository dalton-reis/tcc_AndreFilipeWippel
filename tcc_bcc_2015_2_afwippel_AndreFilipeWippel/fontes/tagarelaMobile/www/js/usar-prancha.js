$(document).ready(function usarPrancha() {
	...
	function transSimbPranchaUso(tx) {
    	for (var i=0; i<9; i++) {
    		tx.executeSql("SELECT categoria,img,audio,id FROM simbolos WHERE id = ?", [simbolosId[i]], okQuerySimbPranchaUso, nokQuery);
    	}
	    ...
	    if (len > 0) {    
	        for (var i=0; i<len; i++) {
		    	...
		    	if (localStorage.dinamico == "iOS") {
	    			$(".simbolos").append("<li style='display:inline-block;'>"
									 +"<img src='img/"+results.rows.item(i).img+"' title='"+results.rows.item(i).audio+"' alt='"+results.rows.item(i).id+"' class='img-simbolo' style='margin:20px; border:10px solid "+corBorda+"' height='150' width='150'/>"
									 +"</li>");
		    	}
	    		else {
		    		$(".simbolos").append("<li style='display:inline-block;'>"
									 +"<img src='"+results.rows.item(i).img+"' title='"+results.rows.item(i).audio+"' alt='"+results.rows.item(i).id+"' class='img-simbolo' style='margin:20px; border:10px solid "+corBorda+"' height='150' width='150'/>"
									 +"</li>");
		    	}
		    }
		}
			...
			if (localStorage.dinamico == "iOS") {
				$(".simbolos").append("<li style='display:inline-block;'>"
									+"<img src='img/no-symbol.png' title='audio/no-sound.mp3' alt='0' class='img-simbolo' style='margin:20px' height='150' width='150'/>"
									+"</li>");
			}
			else {
				$(".simbolos").append("<li style='display:inline-block;'>"
	...
	function transInsUso(tx) {
    	if (localStorage.gravarLog != "false") {
			tx.executeSql("INSERT INTO log (builder,prancha,data_hora,atividade,sinc) VALUES (?,?,?,?,1)", [builderUso,localStorage.idPrancha,dataHora,atv]);		
		}
	}
