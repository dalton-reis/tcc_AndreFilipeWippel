$(document).ready(function() {
    ...
    function okQuerySimbsUsu(tx, results) {
        ...
        if (len > 0) {
	        for (var i=0; i<len; i++) {
	            $(".simbolos").append("<li class='simbs' style='display:inline-block'><a href='#'>"
									 +"<img src='img/"+results.rows.item(i).img+"' title='"+results.rows.item(i).audio+"' alt='"+results.rows.item(i).id+"' class='img-simb' style='margin:20px' height='150' width='150'/>"
								  	 +"</a></li>");
	        }
	        $(".img-simb").click(function cadastrarUsuario() {
				alert("ini img-simb.click");
				var alt = $(this).attr("alt");
				idSimbUsuario = Number(alt);
				db.transaction(transUsuUsu, nokQuery);
				alert("fim img-simb.click");
			});
	    }
	    else {
	    	alert("Não há símbolos nesta categoria!");
    ...
    function transUsuUsu(tx) {
    	tx.executeSql("INSERT INTO usuarios (usuario,senha,perfil,simbolo) VALUES (?,?,?,?)", [localStorage.usuarioSel,localStorage.senhaSel,localStorage.perfilSel,idSimbUsuario], okQueryUsuUsu, nokQuery);
    }
	...
	function transInfoUsu(tx) {
    	tx.executeSql("INSERT INTO info (usuario,nome,email,telefone) VALUES (?,?,'','') ", [id,localStorage.usuarioSel]);
